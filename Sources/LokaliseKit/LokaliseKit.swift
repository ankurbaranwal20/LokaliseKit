...
public enum LokaliseKit {
    ...
    public static func initialize(urlString: String, languageCode: String, completion: @escaping () -> Void) {
        let vm = LocalizationViewModel()
        let token = UUID()
        activeViewModels[token] = vm
        vm.getLocalizationData(urlString: urlString, currentLanguageCode: languageCode) {
